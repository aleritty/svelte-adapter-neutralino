...
export default function (options = defaultOptions) {
  return {
    ...
    async adapt(builder) {
      ...
      console.log(
        chalk.bgYellow(" Building ") + " Downloading Neutralinojs dependencies"
      );
      execSync('npx --quiet "@neutralinojs/neu@' + cliVersion + '" update', {
        cwd: tmpPath,
      });
      console.log(
        chalk.bgYellow(" Building ") + " Generating Neutralinojs release"
      );
      execSync(
        'npx --quiet "@neutralinojs/neu@' + cliVersion + '" build --release',
        { cwd: tmpPath }
      );
      ...
      console.log(
        chalk.bgGreen(" Success ") +
          ...
          chalk.cyan(resolve(options.output))
      );
    },
  };
}
